...
let UsersService = class UsersService {
    ...
    async checkPassword(id, password) {
        ...
        if (!user)
            throw new common_1.NotFoundException('کاربر مورد نظر پیدا نشد');
    ...
    async changePass(id, passwordData, issuedUser) {
        ...
        const checkPass = issuedUser.role == roles_enum_1.Roles.Admin
            ? true
            : await this.validatePassword(user.password, passwordData.current);
        if (issuedUser.role !== roles_enum_1.Roles.Admin && !checkPass)
    ...
    async validatePassword(hashPassword, plainPassword) {
